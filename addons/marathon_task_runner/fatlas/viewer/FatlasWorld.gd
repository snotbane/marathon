...
@export var edited_resource: CompositeTexture2D:
	...
	set(value):
		...
		(pixie.mesh as QuadMesh).size = Vector2(
			minf(1.0, float(_edited_resource.map_default.get_size().x) / _edited_resource.map_default.get_size().y),
			minf(1.0, float(_edited_resource.map_default.get_size().y) / _edited_resource.map_default.get_size().x),
		)
		...
		for k in _edited_resource.maps:
			...
			material.set_shader_parameter(param_size, _edited_resource.map_default.region.size / tex.atlas.get_size())

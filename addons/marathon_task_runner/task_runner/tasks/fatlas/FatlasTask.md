...
> [!TIP]
> The positions of each image in the spritesheet(s) will be completely disorganized and are assigned randomly. It is highly recommended that you have an organized naming scheme for each individual file, so that accessing the resulting images from the FileSystem will be more straightforward. For more information about how `.fat` files are used, consult the [Fatlas Importer](/addons/marathon_task_runner/fatlas/FatlasImporter.md) documentation.
...
![Fatlas Layout](/addons/marathon_task_runner/readme/fatlas_layout.png)
